...
import { InsuranceCategory, lifeInsuranceCategories } from '@/types';

export default function LifeInsuranceQuestionsPage() {
  const router = useRouter();
  const params = useParams();
  const categoryParam = params.category as string;
  const step = parseInt(params.step as string, 10) || 1;

  const category = (lifeInsuranceCategories as readonly InsuranceCategory[]).includes(
  ...
  const goNext = () => {
    if (currentStep < questionList.length) {
      nextStep();
      router.push(`/insurance/life/${category}/questions/${currentStep + 1}`);
    } else {
